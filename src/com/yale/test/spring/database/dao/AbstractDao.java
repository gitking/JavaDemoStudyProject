...
public abstract class AbstractDao<T> extends JdbcDaoSupport {
	...
	public T getById(long id) {
		return this.getJdbcTemplate().queryForObject("SELECT * FROM " + table + " WHERE id = ?", this.rowMapper, id);
	}
	
	public List<T> getAll(int pageIndex) {
		int limit = 100;
		int offset = limit * (pageIndex -1);
		return this.getJdbcTemplate().query("SELECT * FROM " + table + " LIMIT ? OFFSET ? " , new Object[]{limit, offset}, this.rowMapper);
	}
	
	public void deleteById(long id) {
		this.getJdbcTemplate().update("DELETE FROM " + table + " WHERE id = ?", id);
	}
