package com.yale.test.spring.database.tx;
...
import com.yale.test.spring.database.tx.service.User;
import com.yale.test.spring.database.tx.service.UserService;
 ...
 * Spring为啥要抽象出PlatformTransactionManager和TransactionStatus？原因是JavaEE除了提供JDBC事务外，
 * 它还支持分布式事务JTA（Java Transaction API）。分布式事务是指多个数据源（比如多个数据库，多个消息系统）要在分布式环境下实现事务的时候，应该怎么实现。分布式事务实现起来非常复杂，简单地说就是通过一个分布式事务管理器实现两阶段提交，但本身数据库事务就不快，基于数据库事务实现的分布式事务就慢得难以忍受，所以使用率不高。
 ...
 * 使用编程的方式使用Spring事务仍然比较繁琐，更好的方式是通过声明式事务来实现。使用声明式事务非常简单，除了在AppConfig中追加一个上述定义的PlatformTransactionManager外，
 ...
 * 现在问题来了：调用方（比如RegisterController）调用UserService.register()这个事务方法，它在内部又调用了BonusService.addBonus()这个事务方法，
 * 一共有几个事务？如果addBonus()抛出了异常需要回滚事务，register()方法的事务是否也要回滚？
 * 问题的复杂度是不是一下子提高了10倍？
 * 事务传播
 ...
 * 问:廖老师，REQUIRES_NEW这种事务传播方式需要数据库支持吧？我查了下MySQL5.6官方文档，里面提到MySQL不支持事务嵌套，当在一个事务内再次用start transaction开启一个新事务时，
 * 会先隐式的执行commit命令提交之前的事务。那这个REQUIRES_NEW是如何实现的呢？是需要特定数据库的支持吧？或者新打开了一个db连接吗
 * 答:应该是新开一个连接，这样实现起来最简单
 */
@Configuration
@ComponentScan
...
public class AppConfig {
	...
	/*
	 *  org.hsqldb.HsqlException: invalid authorization specification - not found: 
	 *  如果报这个错误就把
	 *  testdb.log
	 * testdb.properties
	 * testdb.script
	...
	public static void main(String[] args) {
		...
		if (userService.fetchUserByEmail("bob@example.com") == null) {
			userService.register("bob@example.com", "password2", "Alice");
		}
