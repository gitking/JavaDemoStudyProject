...
public class StampedLockDemo {
	...
	public void move(double deltaX, double deltaY) {
		...
		System.out.println("写锁的版本号为:" +  stamp);
		try {
			x += deltaX;
			y += deltaY;
		} finally {
	...
	public double distanceFromOrigin() {
		/*
		 * 因为tryOptimisticRead()返回的是版本号，不是锁，根本没有锁
		 * 后面validate()就是为了验证在这段时间内版本号变了没，如果没变，那就没有写入
		 * 版本号就是个long
		 * readLock()才返回真正的读锁，必须finally中unlock
		 */
		long stamp = stampedLock.tryOptimisticRead();//获得一个乐观读锁,但实际上根本没有锁,只是一个版本号
		//注意下面俩行代码不是原子操作
		//假设x,y = (100, 200)
		double currentX = x;
		//此处以读取到x=100,但x,y可能被写线程修改为(300,400)
		double currentY = y;
		//此处已读取到y,如果没有写入,读取时正确的(100, 200)
		//如通有写入,读取时错误的(100, 400)
		if (!stampedLock.validate(stamp)) {//检查乐观读锁读取数据后是否有其他写入发生
			stamp = stampedLock.readLock();//读取一个悲观锁
			/*
			 * 你仔细看for循环的条件，那不是自旋，是反复获取锁
			 * 建议参考jdk9的代码，比较简单
