	...
	public synchronized void get() {
		while (flag) {//不能消费数据了,需要等待生产者生产数据
			try {
				 ...
				 * sleep不管停多久都不会把cpu交出去，pause的话是在优先级大于等于当前线程的lwp ready的时候把cpu调度给他,基本最后的结果就是相同功耗状态下，spinwait能做更多的有效工作,但用了这个之后spinlock被唤醒的时机大概率会延后，但基本就是以cpu的时钟为单位的，可以忽略.
				 * 那延后是因为自旋 语意的原因吧？,是因为可能当前cpu时间片被其他lwp占了.
